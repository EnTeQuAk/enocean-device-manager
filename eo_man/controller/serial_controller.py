    ...
    async def _scan_for_devices_on_bus(self, force_overwrite: bool = False) -> None:
        ...
        try:
            ...
            async for dev in self.enumerate_bus():
                try:
                    # Add retry logic for memory reading
                    retries = 3
                    success = False
                    while retries > 0 and not success:
                        try:
                            ...
                            if retries == 0:
                                logging.error(
                                    f"Read error after retries: Device {dev} announces {dev.discovery_response.memory_size} memory but produces errors at reading"
                                )
                                continue
                    ...
                    logging.error(f"Error processing device {dev}: {str(e)}")
                    continue
