...
class SerialPortDetector:
    ...
    async def async_get_gateway2serial_port_mapping(self) -> dict[str : list[str]]:
        ...
        # Get list of available ports on all platforms
        ports = [d.device for d in serial.tools.list_ports.comports()]

        # Additional check for Linux serial ports if no ports found
        if not ports and not sys.platform.startswith("win"):
            import glob

            # Common Linux serial port patterns
            patterns = [
                "/dev/ttyUSB*",  # USB-to-Serial adapters (most common for Eltako)
                "/dev/ttyACM*",  # USB CDC ACM devices
                "/dev/ttyS*",  # Built-in serial ports
                "/dev/serial/by-id/*",  # Persistent serial port names
            ]
        ...
        for baud_rate in [9600, 57600]:
            for port in ports:
                ...
                try:
                    ...
                    try:
                        # Only attempt RS485 configuration if supported
                        s.rs485_mode = serial.rs485.RS485Settings()
                    except Exception as e:
                        # Ignore RS485 configuration errors - device might still work
                        logging.getLogger().debug(
                            f"RS485 mode not supported on {port}: {str(e)}"
                        )
                        pass
                    s.close()
                    ...
                    print(f"catched exception when configuring port {port}", e)
                    pass
