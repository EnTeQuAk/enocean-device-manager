...
class PCT14DataManager:
    ...
    async def get_devices_from_pct14(cls, filename: str) -> dict:
        ...
        # detect fam14 first
        fam14_device: Device = await cls._create_fam14_device(
            pct14_import["rootdevice"]
        )
        ...
        for d in pct14_import["devices"]["device"]:
            ...
            for i in range(1, dev_size + 1):
                ...
                for si in device.memory_entries:
                    ...
                    if device.is_ftd14():
                        s2: Device = Device.get_decentralized_device_by_sensor_info(
                            si, device.additional_fields["second base id"]
                        )
    ...
    @classmethod
    async def write_sender_ids_into_existing_pct14_export(
        cls,
        source_filename: str,
        target_filename: str,
        ...
        for d in data_dict["exchange"]["devices"]["device"]:
            ...
            for i in range(1, dev_size + 1):
                ...
                if external_id in devices:
                    ...
                    if (
                        "sender" in _d.additional_fields
                        and not cls._is_device_registered(
                            _d, d, i, base_id, function_id
                        )
                    ):
                        # check if HA sender is registered
                        cls._add_ha_sender_id_into_pct14_xml(
                            base_id, d, d["data"]["rangeofid"]["entry"], _d
                        )
                    else:
                        LOGGER.debug(
                            f"PCT14 Export Extender: device {_d.name} ('{_d.external_id}' already registered in actuator {d['name']['#text']})"
                        )
    ...
    def _add_ha_sender_id_into_pct14_xml(
        cls, base_id, xml_device, xml_entries, device: Device
    ):
        index_range = data_helper.find_device_class_by_name(
            xml_device["name"]["#text"]
        ).sensor_address_range
        free_index = -1
        ...
        if free_index > -1:
            ...
            LOGGER.debug(
                f"PCT14 Export Extender: Added sender id '{sender_id}' to PCT14 export for device {device.name} '{device.external_id}'."
            )
        else:
            LOGGER.warning(
                f"PCT14 Export Extender:  Cannot add sender id '{sender_id}' entry into device  {device.name} '{device.external_id}'."
            )

    @classmethod
    def _get_sender_id(cls, base_id: str, sender_id: str):
        sender_offset_id = base_id
        str_id = data_helper.a2s(
            int(sender_id, base=16) + data_helper.a2i(sender_offset_id)
        )
        int_id = int("".join(str_id.split("-")[::-1]), base=16)
        return int_id

    @classmethod
    def _is_device_registered(
        cls, device: Device, pct14_device, channel: int, base_id: str, function_id: int
    ):
        if not isinstance(pct14_device["data"]["rangeofid"]["entry"], list):
            return False

        ha_id = add_addresses(
            base_id, "00-00-00-" + device.additional_fields["sender"]["id"]
        )
        ...
        for e in pct14_device["data"]["rangeofid"]["entry"]:
            ...
            is_registered = is_registered and (
                b2s(cls._convert_sensor_id_to_bytes(e["entry_id"])) == ha_id
            )
            if is_registered:
                return True
        return False
    ...
    async def _create_device(cls, import_obj, fam14: Device, channel: int = 1):
        ...
        if "entry" in import_obj["data"]["rangeofid"]:
            bd.memory_entries = cls._get_sensors_from_xml(
                bd, import_obj["data"]["rangeofid"]["entry"]
            )
        else:
            bd.memory_entries = []
